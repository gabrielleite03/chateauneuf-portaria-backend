...
func (r *SQLiteResidentRepository) List(ctx context.Context) ([]domain.Resident, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT unit, owner, phones, tenant, family_members, photo, sync_status, updated_at
		FROM residents
		ORDER BY unit ASC
	`)
	if err != nil {
...
func (r *SQLiteResidentRepository) Upsert(ctx context.Context, resident domain.Resident) (*domain.Resident, error) {
	...
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO residents (
			unit, owner, phones, tenant, family_members, photo, sync_status, sync_error, created_at, updated_at, synced_at
		) VALUES (?, ?, ?, ?, ?, ?, ?, '', ?, ?, ?)
	...
	`, resident.Unit, resident.Owner, resident.Phones, resident.Tenant, resident.FamilyMembers,
